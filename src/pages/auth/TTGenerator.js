...
const TTGenerator = () => {
  ...
  const generatePDF = () => {
    ...
    for (let i = 9; i <= 17; i++) {
      // Loop for each hour from 9:00am to 5:00pm
      ...
      if (time === "12:30" || (time > "12:30" && time < "13:30")) {
        ...
        days.forEach((day) => {
          ...
          const classInfo = classes.find(
            (classInfo) =>
              classInfo.startTime <= time && classInfo.endTime > time
          );
          if (classInfo) {
          ...
          if (classInfo && classInfo.subject !== "No subject") {
            ...
            if (shouldInsertDash) {
              ...
              const availableSubjects = subjects.filter(
                (subject) =>
                  subject.day !== day &&
                  !subjectsPlaced[day].includes(subject.subject)
              );
              if (availableSubjects.length > 0) {
                ...
                row.push(
                  `${selectedSubject.subject} (${selectedSubject.teacher})`
                );
    ...
    <Layout title={"Timetable Generator"}>
      <div className="form-container">
        ...
        <form onSubmit={handleAddClass}>
          ...
          <div className="mb-3">
            <label htmlFor="teacher">Teacher Prefix:</label>{" "}
            {/* New input field for teacher prefix */}
            <input
              type="text"
              name="teacher"
              className="form-control"
          ...
          <button onClick={generatePDF} className="btn btn-success me-2">
            Generate TT
          </button>
          <button
            onClick={generateAutomaticPDF}
            className="btn btn-success me-2"
          >
            Generate Automatic TT
          </button>
          <button
            type="reset"
            onClick={handleReset}
